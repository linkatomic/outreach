// pages-home.jsx — Dashboard home (Member + Lead variants)
...
function LineChart({ data, height = 200, label = 'Emails sent' }) {
...
// Bar chart for daily metric totals
function BarChart({ data, accentLast = true }) {
  const max = Math.max(...data.map(d => d.value), 1);
  return (
    <div>
      <div className="bars" style={{ '--cols': data.length }}>
        {data.map((d, i) => {
          const h = (d.value / max) * 100;
          const isLast = i === data.length - 1;
          return (
            <div key={i}
                 className={`col ${accentLast && isLast ? 'accent' : ''}`}
                 style={{ height: `${h}%` }}
      ...
      {items.map((it, i) => (
        <div className="feed-row" key={i}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div className="meta">
              <span className={`chip ${it.chip.tone}`}>{it.chip.label}</span>
            </div>
          </div>
...
// ──────────────── MEMBER HOME ────────────────
function MemberHome({ me, setRoute }) {
    ...
    <div className="page">
                  ...
                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(26, 1fr)', gap: 2 }}>
                      {Array.from({ length: 26 }).map((_, i) => {
                        ...
                        return <div key={i} className={`cell lvl${lvl}`} style={{ aspectRatio: '1 / 1', background: lvl === 0 ? 'var(--surface-2)' : `rgba(210, 254, 92, ${0.15 + lvl * 0.2})`, borderRadius: 2 }} title={`${fmtDateShort(date)}: ${total}`}></div>;
                      })}
                    </div>
                  </div>
                );
      ...
      <div className="card">
        ...
        <div style={{ padding: '0 16px' }}>
          {teamMembers.map((m, i) => {
            const score = emailsCountByDay(m.id, 7).reduce((s, d) => s + d.count, 0);
            return { m, score };
...
Object.assign(window, { MemberHome, LeadHome, Sparkline, LineChart, BarChart, ActivityFeed });
