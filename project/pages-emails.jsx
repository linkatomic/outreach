// pages-emails.jsx — Email Log (3 modes: quick-add, grid, bulk paste)

function EmailLogPage({ me, setRoute, showToast, focusEmailOnMount, bulkPasteOnMount }) {
  const [mode, setMode] = React.useState('quick'); // 'quick' | 'grid' | 'bulk'
  const [vendor, setVendor] = React.useState('');
  const [link, setLink] = React.useState('');
  const [filter, setFilter] = React.useState('today'); // today | week | all
  const [filterMember, setFilterMember] = React.useState('all'); // 'all' | memberId | 'me'
  const [search, setSearch] = React.useState('');
  const [bulkText, setBulkText] = React.useState('');
  const [extraRows, setExtraRows] = React.useState([]); // user-added entries
  ...
  React.useEffect(() => { if (bulkPasteOnMount) setBulkOpen(true); }, [bulkPasteOnMount]);
  React.useEffect(() => { if (focusEmailOnMount && quickRef.current) quickRef.current.focus(); }, [focusEmailOnMount]);
  ...
  // Duplicate detection (within last 7d, same vendor + same person)
  ...
  function submitBulk() {
    ...
    const parsed = lines.map((line, i) => {
      // Detect: "Vendor Name — missive.app/123" or just URL
...
Object.assign(window, { EmailLogPage });
