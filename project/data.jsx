...
// Metric registry — used by the daily-report numpad
const METRICS = [
...
function rndInt(seed, min, max) { return Math.floor(rnd(seed) * (max - min + 1)) + min; }

// ─────────── Date helpers ───────────
function todayISO() { return new Date('2026-05-26').toISOString().slice(0, 10); }
...
// ─────────── Seeded report data ───────────
// 30 days of reports for each team member
...
function teamEmailsCountByDay(days = 14) {
  const out = [];
  for (let d = days - 1; d >= 0; d--) {
    const date = isoNDaysAgo(d);
...
Object.assign(window, {
  TEAM, METRICS, METRIC_GROUPS, VENDORS, REPORTS, EMAILS, ICONS,
  Icon, hash, rnd, rndInt,
  todayISO, isoNDaysAgo, fmtDateShort, fmtDayName, fmtFull, fmtRel,
  reportsForMember, emailsForMember, emailsCountByDay, teamEmailsCountByDay,
  metricTotal, reportToday, emailsToday, pct,
});
