// pages-rest.jsx — Analytics, Team/Leaderboard, Review Queue, Settings, Member detail

// ──────────────── ANALYTICS ────────────────
function AnalyticsPage({ setRoute }) {
    ...
    <div className="page">
      ...
      <div className="grid grid-4" style={{ marginBottom: 16 }}>
        ...
        <div className="kpi">
          ...
          <div className="kpi-target">Reports on time, last {days}</div>
        </div>
      </div>
      ...
      {/* Member comparison + Metric breakdown */}
      <div className="grid grid-2" style={{ marginBottom: 16 }}>
        ...
        <div className="card">
          ...
          <div style={{ padding: '0 16px 16px' }}>
            {metricBreakdown.map(({ mt, total }, i) => (
...
function TeamPage({ role, me, setRoute, openDetailFor }) {
  const members = TEAM;
    ...
    <div className="page">
      ...
      <div className="grid grid-3">
        {members.map(m => {
          ...
          const todayEmails = emailsToday(m.id);
            ...
            <div key={m.id} className="card" style={{ padding: 16, cursor: 'pointer' }} onClick={() => openDetailFor(m.id)}>
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
                <div>
                  ...
                  <div className="mono" style={{ fontSize: 16, fontWeight: 500 }}>{todayEmails}<span className="faint" style={{ fontSize: 11 }}>/30</span></div>
                </div>
                <div>
...
function LeaderboardPage({ setRoute, openDetailFor }) {
  const [scope, setScope] = React.useState('week');
  const [metric, setMetric] = React.useState('emails');
      ...
      <div className="card">
        <div className="card-pad">
          {scored.map(({ m, score, emails, sites }, i) => (
            <div key={m.id} style={{ display: 'grid', gridTemplateColumns: '40px 1fr 1fr 90px', gap: 16, padding: '14px 4px', borderBottom: i < scored.length - 1 ? '1px solid var(--border)' : 'none', alignItems: 'center' }}
                 onClick={() => openDetailFor(m.id)}>
              <div className={`rank ${i === 0 ? 'top' : ''}`} style={{ fontFamily: 'var(--font-mono)', fontSize: 14, width: 28, height: 28, display: 'grid', placeItems: 'center', borderRadius: 6, background: i === 0 ? 'var(--accent)' : 'var(--surface-2)', color: i === 0 ? 'var(--accent-ink)' : 'var(--text-dim)', fontWeight: 600 }}>{i + 1}</div>
...
// ──────────────── REVIEW QUEUE (Lead only) ────────────────
function ReviewPage({ setRoute, showToast }) {
  ...
  const [selected, setSelected] = React.useState(filed[0]?.r?.id);
...
// ──────────────── MEMBER DETAIL PANEL ────────────────
function MemberDetailPanel({ memberId, onClose, setRoute }) {
      ...
      <div className="dp-body">
        <div className="grid grid-3" style={{ marginBottom: 16 }}>
          <div className="kpi" style={{ padding: 12 }}>
            ...
            <div className="kpi-value" style={{ fontSize: 22 }}>{todayEmails}</div>
          </div>
          <div className="kpi" style={{ padding: 12 }}>
...
// ──────────────── SETTINGS ────────────────
function SettingsPage({ theme, toggleTheme, role }) {
  return (
...
Object.assign(window, { AnalyticsPage, TeamPage, LeaderboardPage, ReviewPage, MemberDetailPanel, SettingsPage });
